...
export default {
  ...
  data () {
    return {
      ...
      converHtml: '',
      trackName: 'utm_source=kiehls-edm&utm_medium=email&utm_campaign'
    }
  },
  created () {
    this.onShowButton()
  ...
  methods: {
    ...
    async changeImg () {
      ...
      if (this.selectedImg.sku && this.selectedImg.img.parent().attr('href')) {
        ...
        const a = $(`<a href="http://www.kiehls.com.cn/product/${this.selectedImg.sku}.html?${this.trackName}=" target="_blank"></a>`)
    ...
    addSku () {
      ...
      const aProduct = $(`<a href="http://www.kiehls.com.cn/product/${this.selectedImg.sku}.html?${this.trackName}=" target="_blank"></a>`)
      const aLp = $(`<a href="https://www.kiehls.com.cn/landing/${this.selectedImg.sku}.html?${this.trackName}=" target="_blank"></a>`)
    ...
    addTrack () {
      ...
      const homePage = $(`<a href="https://www.kiehls.com.cn/?${this.trackName}=${this.selectedImg.imgTrack}" target="_blank"></a>`)
    ...
    createSingleTd (sku, imgTrack, newImg) {
      const td = $('<td></td>')
      const productALink = $(`<a href="https://www.kiehls.com.cn/product/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const lpALink = $(`<a href="https://www.kiehls.com.cn/landing/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const homePage = $(`<a href="http://www.kiehls.com.cn/?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      if (sku) {
        // 若填写了sku
        // 则跳转到产品详情页
        if (this.addImgUrl === 'product') {
    ...
    createSingleTr (sku, imgTrack, newImg, addImgWay) {
      const tr = $('<tr></tr>')
      let td = $('<td align="center" width="700"></td>')
      if (addImgWay === 'addMultiple') {
        td = $('<td></td>')
      }
      const productALink = $(`<a href="https://www.kiehls.com.cn/product/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const lpALink = $(`<a href="https://www.kiehls.com.cn/product/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const homePage = $(`<a href="http://www.kiehls.com.cn/?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      if (sku) {
        // 若填写了sku
        // 则跳转到产品详情页
