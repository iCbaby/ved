  ...
  <div id="dw-control-panel">
    ...
    <div v-if="showPanel && handleSuccess" class="img-panel" :style="styleObject">
      <div class="topTips">
        ...
        <span>若要跳转到官网首页，要填写监测信息，无需填写code </span>
      </div>
      ...
      <div class="panel-rows">
        ...
        <el-select v-model="addImgPosition" placeholder="请选择" :disabled="selectedImg.imgStruts === 'single'" size="mini" style="width: 18%; margin-bottom: 5px; margin-left: 8px">
          ...
          <div>
            <div class="panel-rows">
              <el-input v-model="addImg.imgSrc" size="mini" placeholder="需要添加的图片src" style="width:20%; margin-left: 8px" />
              <el-input
                v-model="addImg.sku"
                size="mini"
                placeholder="跳转的code(若需添加链接)"
                style="width:27%;margin-left:8px"
              />
...
export default {
  ...
  data () {
    return {
      ...
      converHtml: '',
      trackName: 'utm_source=gac-edm&utm_medium=email&utm_campaign'
    }
  },
  created () {
    this.onShowButton()
  ...
  methods: {
    ...
    async changeImg () {
      ...
      if (this.selectedImg.sku && this.selectedImg.img.parent().attr('href')) {
        ...
        const a = $(`<a href="https://www.giorgioarmanibeauty.cn/product/${this.selectedImg.sku}.html?${this.trackName}=" target="_blank"></a>`)
    ...
    addSku () {
      ...
      const aProduct = $(`<a href="https://www.giorgioarmanibeauty.cn/product/${this.selectedImg.sku}.html?${this.trackName}=" target="_blank"></a>`)
      const aLp = $(`<a href="https://www.giorgioarmanibeauty.cn/landing-pages/${this.selectedImg.sku}.html?${this.trackName}=" target="_blank"></a>`)
      if (this.addImgUrl === 'product') {
        this.selectedImg.img.wrap(aProduct)
        this.selectedImg.isLinkImg = true
        this.$message.success(`已添加新链接，跳转至${this.selectedImg.sku}产品详情页`)
      } else if (this.addImgUrl === 'landingPage') {
        this.selectedImg.img.wrap(aLp)
        this.selectedImg.isLinkImg = true
        this.$message.success(`已添加新链接，跳转至${this.selectedImg.sku}Lp页面`)
      }
    },
    // 修改图片链接sku
    changeSku () {
    ...
    addTrack () {
      ...
      const homePage = $(`<a href="https://www.giorgioarmanibeauty.cn/?${this.trackName}=${this.selectedImg.imgTrack}" target="_blank"></a>`)
    ...
    createSingleTd (sku, imgTrack, newImg) {
      const td = $('<td></td>')
      const productALink = $(`<a href="https://www.giorgioarmanibeauty.cn/product/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const lpALink = $(`<a href="https://www.giorgioarmanibeauty.cn/landing-pages/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const homePage = $(`<a href="https://www.giorgioarmanibeauty.cn/?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      if (sku) {
        // 若填写了sku
        // 跳转到产品详情页
        if (this.addImgUrl === 'product') {
    ...
    createSingleTr (sku, imgTrack, newImg, addImgWay) {
      const tr = $('<tr></tr>')
      let td = $('<td width="700" align="center"></td>')
    ...
    createaddMultipleTr (sku, imgTrack, newImg) {
      ...
      const table = $('<table width="700" border="0" cellpadding="0" cellspacing="0" align="center"></table>')
      const insideTr = $('<tr></tr>')
      const insieTd = $('<td width="700"></td>')
      const productALink = $(`<a href="https://www.giorgioarmanibeauty.cn/product/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const lpALink = $(`<a href="https://www.giorgioarmanibeauty.cn/landing-pages/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const homePage = $(`<a href="https://www.giorgioarmanibeauty.cn/?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      if (sku) {
