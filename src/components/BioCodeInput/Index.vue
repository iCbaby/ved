<template>
  <div id="app-code-input">
    ...
    <!-- 新建BIO PDP dialog -->
    <el-dialog title="新建BIO PDP" :visible.sync="createPdpdialogVisible" width="30%" :before-close="handleClose">
      <div>
        <h5 style="color: #f13b3b">文件夹路径 如：/detail2/1/</h5>
        <el-input v-model="pdpUrlInput" size="mini" placeholder="请输入文件夹路径" />
        <h5 style="color: #f13b3b">图片名称请输入序号前的名称 所有图片序号必须从01开始递增</h5>
        <h5 style="color: #f13b3b">pc端图片名称 如：BIO20007_01.jpg 则输入 BIO20007_</h5>
        <h5 style="color: #f13b3b">mb端图片名称 如：BIO20007m_01.jpg 则输入 BIO20007m_</h5>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="createBioPdp">确定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  computed: {
    pdpTemplate () {
      return `<link rel="stylesheet" type="text/css" href="lib/css/BIOcommon.css?$staticlink$">
<!-- 1.Internal Image : <img src="xxx.jpg?$staticlink$"/> -->
<!-- 2.Category landing page: <a href="$url('Search-Show','cgid','makeup')$"></a> -->
<!-- 3.Product Detail page ：<a href="$url('Product-Show','pid','BIO00091')$"></a> -->
