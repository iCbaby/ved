...
export default {
  ...
  data () {
    return {
      ...
      dialogCode: '',
      skuCode: '',
      urlPrefix: 'https://staging-cn-loreal.demandware.net/on/demandware.static/-/Sites-armani-cn-Library/default',
    ...
    genarateCode () {
      ...
      this.$nextTick(() => {
        $('.conver-html').find('img').each(function () {
          const _this = $(this)
          const _thisParent = _this.parent()
          if (_thisParent.hasClass('por') && !_thisParent.hasClass('landing_pc')) {
            _this.removeAttr('data-msrc')
          } else if (_thisParent.hasClass('mb-por') && !_thisParent.hasClass('landing_mobile')) {
