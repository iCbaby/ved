...
export default {
  ...
  data () {
    return {
      ...
      converHtml: '',
      trackName: 'utm_source=bio-edm&utm_medium=email&utm_campaign'
    }
  },
  created () {
    this.onShowButton()
  ...
  methods: {
    ...
    async changeImg () {
      ...
      if (this.selectedImg.sku && this.selectedImg.img.parent().attr('href')) {
        ...
        const a = $(`<a href="https://www.biotherm.com.cn/product/${this.selectedImg.sku}.html?${this.trackName}=" target="_blank"></a>`)
    ...
    addSku () {
      ...
      const aProduct = $(`<a href="https://www.biotherm.com.cn/product/${this.selectedImg.sku}.html?${this.trackName}=" target="_blank"></a>`)
      const aLp = $(`<a href="https://www.biotherm.com.cn/landing/${this.selectedImg.sku}.html?${this.trackName}=" target="_blank"></a>`)
    ...
    addTrack () {
      ...
      const homePage = $(`<a href="http://www.biotherm.com.cn/?${this.trackName}=${this.selectedImg.imgTrack}" target="_blank"></a>`)
    ...
    createSingleTd (sku, imgTrack, newImg) {
      const td = $('<td></td>')
      const productALink = $(`<a href="https://www.biotherm.com.cn/product/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const lpALink = $(`<a href="https://www.biotherm.com.cn/landing/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const homePage = $(`<a href="http://www.biotherm.com.cn/?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      if (sku) {
        // 若填写了sku
        // 跳转到产品详情页
        if (this.addImgUrl === 'product') {
    ...
    createSingleTr (sku, imgTrack, newImg, addImgWay) {
      const tr = $('<tr></tr>')
      let td = $('<td align="center" width="608"></td>')
    ...
    createaddMultipleTr (sku, imgTrack, newImg) {
      ...
      const table = $('<table width="608" border="0" cellpadding="0" cellspacing="0" align="center"></table>')
      const insideTr = $('<tr></tr>')
      const insieTd = $('<td width="608"></td>')
      const productALink = $(`<a href="https://www.biotherm.com.cn/product/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const lpALink = $(`<a href="https://www.biotherm.com.cn/landing/${sku}.html?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      const homePage = $(`<a href="http://www.biotherm.com.cn/?${this.trackName}=${imgTrack}" target="_blank"></a>`)
      if (sku) {
